...
<?php $__env->startSection('title', 'Tournament Match Scoring'); ?>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="tournamentGrid">
                ...
                <div class="tournament-card bg-gradient-to-br from-white to-blue-50 rounded-xl border-2 border-gray-200 hover:border-blue-300 transition-all hover:shadow-xl cursor-pointer p-6"
                     ...
                     onclick="selectTournament(<?php echo e($tournament->id); ?>, '<?php echo e($tournament->name); ?>')">
                    
                    <!-- Tournament Info -->
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            <h3 class="font-bold text-lg text-black mb-1"><?php echo e($tournament->name); ?></h3>
                            <div class="text-sm text-black space-y-1">
                                <div class="flex items-center gap-2">
                                    <span class="text-blue-600">📅</span>
                                    <?php echo e($tournament->start_date ? \Carbon\Carbon::parse($tournament->start_date)->format('M j, Y') : 'TBD'); ?>

                                </div>
                                <?php if($tournament->end_date && $tournament->start_date !== $tournament->end_date): ?>
                                <div class="flex items-center gap-2">
                                    <span class="text-blue-600">📅</span>
                                    to <?php echo e(\Carbon\Carbon::parse($tournament->end_date)->format('M j, Y')); ?>
                        ...
                        <div class="ml-4">
                            ...
                            ?>
                            <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border <?php echo e($statusClass); ?>">
                                <?php echo e($statusEmoji); ?> <?php echo e(ucfirst($tournament->status ?? 'Upcoming')); ?>
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-200">
                        <div class="text-center">
                            <div class="text-2xl font-bold text-blue-600"><?php echo e($tournament->teams_count ?? 0); ?></div>
                            <div class="text-xs text-black">Teams</div>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl font-bold text-green-600"><?php echo e($tournament->rounds_count ?? 0); ?></div>
...
function selectTournament(tournamentId, tournamentName) {
    window.location.href = `<?php echo e(route('admin.match-scoring.show', '')); ?>/${tournamentId}`;
}
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\EDS-TABULATION\resources\views\admin\match-scoring\index.blade.php ENDPATH**/ ?>
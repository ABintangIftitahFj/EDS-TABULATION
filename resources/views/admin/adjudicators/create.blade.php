...
@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-900">Add New Adjudicator</h1>
        <p class="text-slate-500">Register a new adjudicator for a tournament</p>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="number" name="rating" id="rating" value="{{ old('rating') }}" min="0" max="10" step="0.1"
                        class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    <p class="mt-1 text-sm text-slate-500">Optional: Adjudicator quality rating</p>
                    @error('rating')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>

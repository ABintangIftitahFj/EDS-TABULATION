    ...
    <div class="p-6 text-white">
            ...
            <div class="flex-1">
                ...
                @if($motion->detail)
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 mt-4">
                        <p class="text-sm leading-relaxed">{{ $motion->detail }}</p>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="mt-4 bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm opacity-90">{{ $motion->info_slide }}</p>
                    </div>
                </div>
            </div>
        @endif
        @if($motion->category)
            <div class="mt-3 flex items-center gap-2">
                ...
                @if($motion->released_at)
                    <span class="text-xs opacity-75">
                        Released: {{ $motion->released_at->format('M d, Y H:i') }}
                    </span>
                @endif
            </div>
        @endif
